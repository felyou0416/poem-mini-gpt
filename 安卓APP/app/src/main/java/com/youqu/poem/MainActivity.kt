...
class MainActivity : AppCompatActivity() {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    statusText.text = if (self) "模型自检通过 ✓ 可以开始写诗"
                                      else "模型自检偏差，仍可使用（结果可能略异）"
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
    ...
    private fun onGenerate() {
        if (!ready) { Toast.makeText(this, "模型还没加载完，稍等…", Toast.LENGTH_SHORT).show(); return }
        val seed = seedInput.text.toString().trim()
        if (seed.isEmpty()) { Toast.makeText(this, "先输入一个开头，如：山高水流", Toast.LENGTH_SHORT).show(); return }

        progressBar.visibility = android.view.View.VISIBLE
        progressBar.progress = 0
        statusText.text = "作诗中…（约 20~40 秒）"
        resultBox.removeAllViews()

        CoroutineScope(Dispatchers.Default).launch {
            try {
                val poems = generator.generate(seed, 0.6f, 3)
                withContext(Dispatchers.Main) {
                    progressBar.visibility = android.view.View.GONE
                    if (poems.isEmpty()) {
                        statusText.text = "这轮没生成合格诗作，换个开头试试？"
                        return@withContext
                    }
                    statusText.text = "完成 ✓ 共 ${poems.size} 首"
